 ...
 */
function parseInteractiveUrl(url) {
    // Wordwall
...
function getPlatformConfig(platform, id) {
    const configs = {
        wordwall: {
        ...
        notebooklm: {
            ...
            embedHtml: `<div class="bg-indigo-50 p-8 rounded-3xl border-2 border-indigo-100 text-center"><h3 class="text-xl font-bold text-indigo-900 mb-4">NotebookLM AI Overview</h3><p class="text-indigo-600 mb-6 font-medium">Kliknij poniżej, aby otworzyć interaktywny notatnik AI z materiałami do tej lekcji.</p><a href="https://notebooklm.google.com/notebook/${id}" target="_blank" class="inline-flex items-center justify-center px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg hover:bg-indigo-700 transition-colors">Otwórz NotebookLM</a></div>`,
            category: 'ai-tools'
        }
    };
...
function syncInteractive(urls) {
    ...
    const existingUrls = new Set(products.map(p => p.source?.url).filter(Boolean));

    let nextIdNum = Math.max(...products.map(p => {
    ...
    urls.forEach(url => {
        ...
        if (!info) {
            console.warn('Could not detect platform or ID from URL:', url);
            return;
        }

        const config = getPlatformConfig(info.platform, info.id);
        if (existingUrls.has(config.canonicalUrl)) {
            console.log(`Skipping already imported ${info.platform} product:`, config.canonicalUrl);
            return;
        }
        ...
        title = title || `${info.platform} Activity ${info.id}`;

        // Clean up title
        title = title.split('?')[0].replace(/[_-]/g, ' ');
        title = title.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');

        const desc = `Zasób edukacyjny ${info.platform}: ${title}. Skuteczne narzędzie dla każdego nauczyciela.`;
        ...
        const newProduct = {
            ...
            description: desc,
            price: 15,
            url: `https://www.sklep.linguachess.com/index.php/product/${slugify(title)}/`,
            categories: ["interactive-game", config.category],
        ...
        existingUrls.add(config.canonicalUrl);
        console.log(`Added new ${info.platform} product:`, title);
    });

    if (newProducts.length > 0) {
        const updatedProducts = [...products, ...newProducts];
        fs.writeFileSync(PRODUCTS_PATH, JSON.stringify(updatedProducts, null, 2), 'utf8');
        console.log(`Successfully synced ${newProducts.length} new products.`);
    } else {
...
if (args.length > 0) {
    syncInteractive(args);
}

module.exports = { syncInteractive, parseInteractiveUrl };
